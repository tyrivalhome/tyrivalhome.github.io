/**
 ...
 (function () {
    return {
        style: {},      // 对象，声明图表的基本样式，可为空。
        option: {},     // 对象，声明图表本身的配置，必填。
        data: [],       // 数组，图表载入的默认数据，必填。
        event: [],      // 数组，图表的事件注册，可为空。
        proxy: {},      // 对象，图表默认的数据源配置，可为空。
        transform: {},  // 对象，数据映射规则，必填。
        trigger: [],    // 数组，图表使用的触发器，可为空。
        handler: function(config)   // 函数，将 data 数据转化为合适的格式，并赋值给 option 中的相应属性，可为空。
    }
 })()
 */
(function () {
    return {
         ...
         * 藏，组件从远端接口重新获取数据。
         * 其中各属性的含义如下：
         * - name: Echarts的图表事件，详细参考：https://echarts.apache.org/zh/api.html#events.鼠标事件
        ...
        /**
         * handler 表示图表的所有配置都加载完成后（包括从远端获取数据），在图表渲染之前，所做的最后一次操作，
         * 通常，远端获取的数据属性与 transform 的默认属性差距较大，需要根据 transform.mapper 中的映射
         * 规则，对数据进行转换处理，并赋予 option 中的相应属性，然后才可使图表成功渲染。
